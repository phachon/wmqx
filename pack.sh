...
tar -czvf "${RELEASE}/wmqx-linux-amd64.tar.gz" wmqx config.toml message.json log/.gitignore License.md README.md README_CN.md
rm -rf wmqx

echo 'Start pack linux 386'
GOOS=linux GOARCH=386 go build -ldflags "-s -w"
tar -czvf "${RELEASE}/wmqx-linux-386.tar.gz" wmqx config.toml message.json log/.gitignore License.md README.md README_CN.md
rm -rf wmqx

echo 'Start pack mac amd64'
GOOS=darwin GOARCH=amd64 go build -ldflags "-s -w"
tar -czvf "${RELEASE}/wmqx-mac-amd64.tar.gz" wmqx config.toml message.json log/.gitignore License.md README.md README_CN.md
rm -rf wmqx
